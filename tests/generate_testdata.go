// Package main generates gob test fixtures for the gobts test suite.
...
func main() {
	...
	// --- Scalar bytes ---
	// json.Marshal encodes []byte as base64; TypeScript tests must base64-decode.
	...
	// the wire, so the decoder must supply TypeScript zero values for missing fields.
	...
	// map[int]string: JSON object keys are always strings, so the TypeScript test
